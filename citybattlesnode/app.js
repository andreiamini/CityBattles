...
app.post('/geolocation', function(req, res){
    if(Parse.User.current() && req.body.city != undefined){
        Parse.User.current().fetch().then(function (user) {
        console.log(req.body.city + " is your city");
        name = user.getUsername();
        console.log(name + " is the user");
        user.set("city", req.body.city);
        user.save();
        //see if the city is in the database already, if not add it
        var Cities = Parse.Object.extend("Cities");
        var query = new Parse.Query(Cities);
        query.equalTo("City", req.body.city);
        query.find({
